...
class RNNModelWord(nn.Module):
    ...
    def forward(self, input, hidden, lengths):
        ...
        #print(input)
        emb = self.encoder(input)
        #print(input.size(0), input.size(1),"inp")
        #print(emb, emb.size(0),emb.size(1),emb.size(2), "emb")
        #print(emb)
        #print(emb)
        ...
        #print(output_affine.size(0), output_affine.size(1), self.uw.size(0), self.uw.size(1))
        #print("forward word")
        ...
        #print(word_attn_norm, "SOFTMAX")
        word_attn_vectors = self.attention_mul(output, word_attn_norm)
        #print(word_attn_vectors.size(0), word_attn_vectors.size(1), word_attn_vectors.size(2), "attention vector")
        return word_attn_vectors, hidden, perm_index
    ...
    def init_hidden(self, bsz):
        ...
        return Variable(weight.new(self.nlayers, bsz, self.nhid).zero_())
        #return Variable(torch.zeros(1, bsz, self.nhid))
...
class RNNModelSentence(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        #print(sent_attn_norm, "SHOULD BE SOFTMAX")
